  ...
  String populateTransportSettings({
    ...
    if (transport == 'tcp') {
      ...
      if (headerType == 'http') {
        ...
        if ((host != null && host.isNotEmpty) || (path != null && path.isNotEmpty)) {
          ...
          var hostList = streamSetting['tcpSettings']['header']['request']['headers']['Host'];
      ...
      // 修复：使用httpSettings而不是h2Setting
      streamSetting['httpSettings'] = {
        "host": host?.split(",") ?? [],
        "path": path ?? "/",  // 修复：改为字符串
      };
      var hostList = streamSetting['httpSettings']['host'];
      sni = hostList.length > 0 ? hostList[0] : sni;
    } 
