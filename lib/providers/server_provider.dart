  ...
  Future<void> _ensureMinimumServers() async {
    ...
    try {
      ...
      try {
        ...
        print('从 Cloudflare 获取失败，使用备用节点: $e');
        newServers = CloudflareTestService.getBackupServers(count: needed);
      }
  ...
  Future<void> _initializeWithCloudflareServers() async {
    ...
    try {
      ...
      if (!success || servers.isEmpty) {
        print('所有尝试都失败，使用备用节点');
        _initMessage = '使用备用节点...';
        notifyListeners();
        servers = CloudflareTestService.getBackupServers(count: 5);
      }
      ...
      // 确保至少有5个备用节点
      _servers = _generateNamedServers(CloudflareTestService.getBackupServers(count: 5));
      await _saveServers();
    } finally {
      await Future.delayed(const Duration(seconds: 1));
      _isInitializing = false;
